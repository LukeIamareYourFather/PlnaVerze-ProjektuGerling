...
public class RemovePartiesController {
	...
	public String handeDeletePolicyOwnerFormSubmit(@Validated(OnDeleteParty.class) @ModelAttribute("reasonsDTO") PartiesReasonsFormDTO reasonsDto, BindingResult bindingResult, RedirectAttributes redirectAttributes) {
		...
		if (bindingResult.hasErrors()) {
			redirectAttributes.addFlashAttribute("reasonsDTO", reasonsDto);
		...
		//
		if(reasonsDto.getDateOfRequest().isAfter(LocalDate.now())) {
			redirectAttributes.addFlashAttribute("reasonsDTO", reasonsDto);
			redirectAttributes.addFlashAttribute("org.springframework.validation.BindingResult.reasonsDTO", bindingResult);
	...
	public String renderFindPolicyOwnerToDeleteForm(@ModelAttribute("reasonsDTO") PartiesReasonsFormDTO reasonsDto, Model model) {
		return assignSearchAttributes("formDTO", "formAction", "/parties/delete/found", null, null, model, true, FormNames.PARTY_DELETE.getDisplayName() + " - " + FormNames.PARTY_FIND.getDisplayName().toLowerCase(), null, "pages/parties/find");
	}
	
	@PostMapping("delete/found")
	public String handleFoundPolicyOwnersToDelete(@ModelAttribute("reasonsDTO") PartiesReasonsFormDTO reasonsDto, @ModelAttribute("formDTO") PartiesDetailsDTO partyDto, Model model, RedirectAttributes redirectAttributes) {
		return assignSearchAttributes("foundParties", "referenceLink", "/parties/delete/party-", partyDto, PartyStatus.REGISTERED, model, false, null, redirectAttributes, "pages/parties/found-parties");
	...
	public String handleDeletePolicyOwnerConfirmation(@PathVariable long partyId, @ModelAttribute("reasonsDTO") PartiesReasonsFormDTO reasonsDto, SessionStatus sessionStatus, RedirectAttributes redirectAttributes) {
		processConfirmedRemoval(reasonsDto, partyId, sessionStatus, redirectAttributes);
	 ...
	 */
	private String assignSearchAttributes(String firstKeyName, String secondKeyName, String referenceLink, PartiesDetailsDTO partyDto, PartyStatus partyStatus, Model model, boolean ifFindAssignmentRequested, String formName, RedirectAttributes redirectAttributes, String successRedirectLink) {
	...
	//
	private void processConfirmedRemoval(PartiesReasonsFormDTO reasonsDto, long partyId, SessionStatus sessionStatus, RedirectAttributes redirectAttributes) {
